...
<?php } if ($this->_rootref['S_NO_READ_ACCESS']) {  ?>
	...
	<?php } ?>
		...
		<li class="row<?php if (!($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?> primebg1<?php } else { ?> primebg2<?php } if ($_topicrow_val['S_POST_GLOBAL']) {  ?> global-announce<?php } if ($_topicrow_val['S_POST_ANNOUNCE']) {  ?> announce<?php } if ($_topicrow_val['S_POST_STICKY']) {  ?> sticky<?php } if ($_topicrow_val['S_TOPIC_REPORTED']) {  ?> reported<?php } ?>">
...
<?php } if ($this->_rootref['S_DISPLAY_POST_INFO']) {  ?>
	...
	<div class="stat-bar"><?php echo ((isset($this->_rootref['L_FORUM_PERMISSIONS'])) ? $this->_rootref['L_FORUM_PERMISSIONS'] : ((isset($user->lang['FORUM_PERMISSIONS'])) ? $user->lang['FORUM_PERMISSIONS'] : '{ FORUM_PERMISSIONS }')); ?></div>
	<div style="padding: 5px 9px 9px; margin-bottom: -40px;">
